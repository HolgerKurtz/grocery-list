...
def index():
    ...
    selected_menus = utils.parse_comma_separated_list(
        request.args.get('menus', ''))
    selected_ingredients = utils.parse_comma_separated_list(
        request.args.get('ingredients', ''))

    # Parse boolean parameters
    shopper_mode = request.args.get(
        'shopper', 'false').lower() in ('true', 't', '1', 'yes')
    ...
    logger.info(
        f"Rendering index with {len(selected_menus)} menus and {len(filtered_ingredients)} ingredients")
...
def get_ingredients():
    ...
    logger.info(
        f"Received request for ingredients with {len(selected_menus)} menus")
...
def add_menu():
    ...
    if form.validate_on_submit():
        ...
        new_ingredients = utils.split_ingredients(form.ingredients.data)
        ...
        if not new_ingredients:
            logger.warning(
                f"Attempted to add menu '{new_menu}' with no ingredients")
        ...
        logger.info(
            f"Added new menu '{new_menu}' with {len(new_ingredients)} ingredients")
...
def update_menu(menu_name):
    ...
    if form.validate_on_submit():
        ...
        if not new_ingredients:
            logger.warning(
                f"Attempted to update menu '{menu_name}' with no ingredients")
            return "You must add at least one ingredient."
        ...
        logger.info(
            f"Updated menu '{menu_name}' with {len(new_ingredients)} ingredients")
        return redirect(url_for('index'))
